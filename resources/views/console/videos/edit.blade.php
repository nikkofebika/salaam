		...
		<div class="row">
			<div class="col-xs-12 m-t-md">
				<div class="box box-primary">
					<form method="POST" action="{{ url('console/video_playlists/'.$playlist->id) }}">
						@csrf @method('PUT')
						<div class="box-body">
							<div class="form-group @error('playlist_id') has-error @enderror">
								<label>Playlist ID <span class="text-danger">*</span></label>
								<input type="text" name="playlist_id" class="form-control" required placeholder="Playlist ID" value="{{ $playlist->playlist_id }}">
								@error('playlist_id')
							...
							<div class="form-group @error('title') has-error @enderror">
								...
								<input type="text" name="title" class="form-control" required placeholder="Nama Playlist" value="{{ $playlist->title }}">
								@error('title')
								<span class="help-block">{{ $message }}</span>
								@enderror
							</div>
							<div class="form-group @error('meta_keywords') has-error @enderror">
								...
								<input type="text" name="meta_keywords" class="form-control" required placeholder="Meta Keywords" value="{{ $playlist->meta_keywords }}">
								@error('meta_keywords')
								<span class="help-block">{{ $message }}</span>
								@enderror
							</div>
							<div class="form-group @error('description') has-error @enderror">
								...
								<textarea name="description" class="form-control" rows="5" required placeholder="Deskripsi Playlist">{{ $playlist->description }}</textarea>
								@error('description')
								<span class="help-block">{{ $message }}</span>
								@enderror
							</div>
							<div class="form-group @error('priority') has-error @enderror">
								...
								<input type="number" min="1" name="priority" class="form-control" required placeholder="Urutan Playlist" value="{{ $playlist->priority }}">
								@error('priority')
								<span class="help-block">{{ $message }}</span>
								@enderror
							</div>
							<div class="form-group">
								...
								<div class="radio m-t-xs">
									<label>
										<input type="radio" name="is_active" value="1" @if($playlist->approved_by !== null) checked @endif> Active
									</label>
									<label class="m-l-md">
										<input type="radio" name="is_active" value="0" @if($playlist->approved_by == null) checked @endif> Non Aktif
									</label>
								</div>
							</div>
						</div>
