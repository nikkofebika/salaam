		...
		<div class="row">
			<div class="col-xs-12 m-t-md">
				<div class="box box-primary">
					<form method="POST" action="{{ url('console/items/'.$item->id) }}" enctype="multipart/form-data">
						@csrf @method('PUT')
						<div class="box-body">
							<div class="row">
								<div class="col-md-8 col-sm-12">
									<div class="form-group @error('name') has-error @enderror">
										...
										<input type="text" name="name" class="form-control" required placeholder="Nama Item" value="{{ $item->name }}">
										@error('name')
							...
							<div class="row">
								<div class="col-md-6 col-sm-12">
									<div class="form-group @error('model') has-error @enderror">
										<label>Model</label>
										<input type="text" name="model" class="form-control" placeholder="Model" value="{{ $item->model }}">
										@error('model')
										<span class="help-block">{{ $message }}</span>
										@enderror
									</div>
								...
								<div class="col-md-6 col-sm-12">
									<div class="form-group @error('tag') has-error @enderror">
										<label>Tag</label>
										<input type="text" name="tag" class="form-control" placeholder="Tag" value="{{ $item->tag }}">
										@error('tag')
										<span class="help-block">{{ $message }}</span>
										@enderror
									</div>
							...
							<div class="row">
								...
								<div class="col-md-6 col-sm-12">
									<div class="form-group @error('find_date') has-error @enderror">
										...
										<input type="text" name="find_date" class="form-control" id="datetimepicker" required value="{{ $item->find_date }}" readonly/>
										@error('find_date')
										<span class="help-block">{{ $message }}</span>
										@enderror
									</div>
							...
							<div class="row">
								...
								<div class="col-md-6 col-sm-12">
									<div class="form-group @error('specific_location') has-error @enderror">
										...
										<textarea name="specific_location" class="form-control" required rows="3" placeholder="Lokasi Spesifik">{{ $item->specific_location }}</textarea>
							...
							<div class="row">
								...
								<div class="col-md-6 col-sm-12">
									<div class="form-group @error('regency_id') has-error @enderror">
										...
										<select name="regency_id" class="form-control select2" id="select-regency" required>
											<option value="">- Pilih Kota/Kabupaten -</option>
											@foreach($regencies as $r)
											<option value="{{ $r->id }}" {{ $item->regency_id == $r->id ? "selected" : "" }}>{{ $r->name }}</option>
											@endForeach
										</select>
										@error('regency_id')
										<span class="help-block">{{ $message }}</span>
							...
							<div class="row">
								<div class="col-md-6 col-sm-12">
									<div class="form-group @error('district_id') has-error @enderror">
										...
										<select name="district_id" class="form-control select2" id="select-district" required>
											<option value="">- Pilih Kecamatan -</option>
											@foreach($districts as $d)
											<option value="{{ $d->id }}" {{ $item->district_id == $d->id ? "selected" : "" }}>{{ $d->name }}</option>
											@endForeach
										</select>
										@error('district_id')
										<span class="help-block">{{ $message }}</span>
								...
								<div class="col-md-6 col-sm-12">
									<div class="form-group @error('village_id') has-error @enderror">
										...
										<select name="village_id" class="form-control select2" id="select-village">
											<option value="">- Pilih Kelurahan -</option>
											@foreach($villages as $v)
											<option value="{{ $v->id }}" {{ $item->village_id == $v->id ? "selected" : "" }}>{{ $v->name }}</option>
											@endForeach
										</select>
										@error('village_id')
										<span class="help-block">{{ $message }}</span>
